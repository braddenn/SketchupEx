...
#
def SketchUpEx.printEntities( entities ) # this works
  num = entities.size
...
#
def SketchUpEx.makeQuadDef( length, width, height )
  pts = Array.new
  pts[0] = [0,0,0]
  pts[1] = [length,0,0] # x
  pts[2] = [0, width,0] # y
  ...
  return definition
end # SketchUpEx.makeQuadDef
#
# makePymdDef - add a 5 sided component definition called pyramid
#
def SketchUpEx.makePymdDef( length, width, height )
  pts = Array.new  
  pts[0] = [0,0,0]
  pts[1] = [length,0,0] # x
  ...
  return definition
end # SketchUpEx.makePymdDef
#
# makeCubeDef - add a 6-sided component definition called cube
#
def SketchUpEx.makeCubeDef( length, width, height )
  pts = Array.new
  pts[0] = [0,0,0]
  pts[1] = [length,0,0] # x
  pts[2] = [length, width,0] # xy
...
def SketchUpEx.makeDefs
  $quadDef = SketchUpEx.makeQuadDef( 15, 15, 30 )
  $pymdDef = SketchUpEx.makePymdDef( 15, 15, 30 )
  $cubeDef = SketchUpEx.makeCubeDef( 60, 30, 15 )
end
#
# make one instance of each def
#
def SketchUpEx.makeInstances
...
#
def SketchUpEx.shift( inst, dx, dy, dz )
...
#
def SketchUpEx.rotate( inst, rx, ry, rz, angle )
...
SketchUpEx.shift( inst, -15, -15, -15)

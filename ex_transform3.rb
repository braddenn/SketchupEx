...
#
def makeDefs
	$quadDef = makeQuadDef( 15, 15, 30 )
	$pymdDef = makePymdDef( 15, 15, 30 )
	$cubeDef = makeCubeDef( 60, 30, 15 )
end
#
...
def shift( inst, dx, dy, dz )
	puts "before shift of -15,-15,-15 inst.t is #{inst.transformation.to_a}, origin is #{inst.origin.to_a}"
...
puts "cube origin before rotation is #{inst.origin.to_a}"
UI.messagebox "will rotate cube 22.5 degrees around z"
rotate( inst, 0,0,1, angle )
UI.messagebox "rotated cube 22.5 degrees around z"
puts "cube origin after rotation is #{inst.origin.to_a}"
